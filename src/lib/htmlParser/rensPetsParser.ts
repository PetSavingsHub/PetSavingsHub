...
export function parseHTML(html: string, web?: string) {
  ...
  items.forEach((item) => {
    ...
    const regularPrice = item.querySelector(".product-prices__price--small")?.textContent || item.querySelector(".product-prices__price--autoship")?.textContent || item.querySelector(".product-prices__price")?.textContent;
