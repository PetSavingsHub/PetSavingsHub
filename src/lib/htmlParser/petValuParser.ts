...
export async function parseHtmlPetValu(html: string) {
  ...
  const pages = root
    .querySelector(".page-buttons__wrapper")
    ?.querySelectorAll("a");
  let maxPage = "";
  ...
  itemsATags.forEach((item) => {
    ...
    const nameLeft =
      item.querySelector(".title__left")?.querySelector(".P1.bold")
        ?.textContent || "";
    const nameRight =
      item.querySelector(".title__right")?.querySelector(".P1.regular")
        ?.textContent || "";
    ...
    if (previousPrice || defaultPrice) {
      const regularPriceText =
        previousPrice?.textContent || defaultPrice?.textContent;
